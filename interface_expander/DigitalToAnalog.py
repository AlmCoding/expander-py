...
DAC_GAIN_CH0 = 9.84  # Gain for DAC output
DAC_GAIN_CH1 = 9.82
DAC_OPAMP_OFFSET = DAC_VREF / 2.0  # Op-amp offset for DAC output
DAC_CORRECTION_CH0 = int(0.0238 * DAC_MAX_SAMPLE_VALUE / DAC_GAIN_CH0 / DAC_VREF)  # Channel 0 correction factor
DAC_CORRECTION_CH1 = int(0.0334 * DAC_MAX_SAMPLE_VALUE / DAC_GAIN_CH1 / DAC_VREF)  # Channel 1 correction factor
...
class DacConfig:
    def __init__(self, mode_ch0: DacMode, sampling_rate_ch0: int, sample_count_ch0: int,
                 mode_ch1: DacMode, sampling_rate_ch1: int, sample_count_ch1: int):
...
class DigitalToAnalog:
    def __init__(self):
        ...
        config = DacConfig(mode_ch0=DacMode.STATIC_MODE, sampling_rate_ch0=0, sample_count_ch0=0,
                           mode_ch1=DacMode.STATIC_MODE, sampling_rate_ch1=0, sample_count_ch1=0)
    ...
    def _apply_config(self, config: DacConfig, timeout: float = 1.0) -> DacConfigStatusCode:
        ...
        if self.config and self.config.mode_ch0 == config.mode_ch0 and \
                    self.config.sampling_rate_ch0 == config.sampling_rate_ch0 and \
                    self.config.sample_count_ch0 == config.sample_count_ch0:
                update_ch0_config = False

        if self.config and self.config.mode_ch1 == config.mode_ch1 and \
                    self.config.sampling_rate_ch1 == config.sampling_rate_ch1 and \
                    self.config.sample_count_ch1 == config.sample_count_ch1:
    ...
    def _can_accept_request(self, request) -> bool:
        ...
        if isinstance(request, DacDataRequest):
            if self.queue_space > 0 and \
                    (self.buffer_space_ch0 >= len(request.sequence_ch0) and
        ...
        if accept:
            print("Accept request ok (queue_space: %d, buffer_space_ch0: %d, buffer_space_ch1: %d)" %
    ...
    def output_voltage(self, voltage_ch0: float | None, voltage_ch1: float | None) -> DacDataStatusCode:
        ...
        if voltage_ch0 is not None:
            ...
            value_ch0 = int((voltage_ch0 / DAC_GAIN_CH0 + DAC_OPAMP_OFFSET) * DAC_MAX_SAMPLE_VALUE / DAC_VREF - DAC_CORRECTION_CH0)
        if voltage_ch1 is not None:
            value_ch1 = int((voltage_ch1 / DAC_GAIN_CH1 + DAC_OPAMP_OFFSET) * DAC_MAX_SAMPLE_VALUE / DAC_VREF - DAC_CORRECTION_CH1)
    ...
    def output_value(self, value_ch0: int | None, value_ch1: int | None) -> DacDataStatusCode:
        ...
        if value_ch0 is not None:
            ...
            DigitalToAnalog._verify_parameters(DacMode.STATIC_MODE, sequence_ch0, DAC_MIN_SAMPLING_RATE)
            config.mode_ch0 = DacMode.STATIC_MODE
            config.sampling_rate_ch0 = 0
            config.sample_count_ch0 = 0

        sequence_ch1 = []
        if value_ch1 is not None:
            sequence_ch1 = [value_ch1]
            DigitalToAnalog._verify_parameters(DacMode.STATIC_MODE, sequence_ch1, DAC_MIN_SAMPLING_RATE)
            config.mode_ch1 = DacMode.STATIC_MODE
            config.sampling_rate_ch1 = 0
            config.sample_count_ch1 = 0

        if self._apply_config(config) != DacConfigStatusCode.SUCCESS:
            raise RuntimeError("Failed to apply DAC configuration!")

        request = DacDataRequest(value_ch0 is not None, sequence_ch0,
                                 value_ch1 is not None, sequence_ch1)
    ...
    def loop_sequence(self, sequence_ch1, sequence_ch2, sampling_rate) -> DacDataStatusCode:
        DigitalToAnalog._verify_parameters(DacMode.PERIODIC_MODE, sequence_ch1, sequence_ch2, sampling_rate)
    ...
    def stream_sequence(self, sequence_ch1, sequence_ch2, sampling_rate) -> int:
        ...
        DigitalToAnalog._verify_parameters(DacMode.STREAMING_MODE, sequence_ch1, sequence_ch2, sampling_rate)

        if self.config.mode != DacMode.STREAMING_MODE or self.config.sampling_rate != sampling_rate:
            config = DacConfig(DacMode.STREAMING_MODE, sampling_rate, 0)
            if self._apply_config(config) != DacConfigStatusCode.SUCCESS:
                raise RuntimeError("Failed to apply DAC configuration!")

        # Calculate timeout based on sampling rate
        timeout = DAC_MAX_DATA_SAMPLES / sampling_rate + 0.1  # Time to wait for data to be sampled
        ...
        for i in range(request_count):
            ...
            request = DacDataRequest(True, current_sequence_ch1, current_sequence_ch2)
            rid = self._send_data_request(request, timeout)
            pending_rids.append(rid)
    ...
    def _send_data_request(self, request: DacDataRequest, timeout: float = 0.1) -> int:
        ...
        while True:
            intexp.InterfaceExpander()._read_all()
            if self._can_accept_request(request):
                break

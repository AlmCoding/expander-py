from interface_expander.I2cInterface import I2cInterface, I2cMasterRequest, I2cStatusCode, I2C_MAX_READ_SIZE, I2C_MAX_WRITE_SIZE
...
class Memory:
    def __init__(self, interface: I2cInterface, slave_address: int, memory_type: MemoryType, address_width: MemoryAddressWidth, page_count: int, page_size: int):
    ...
    def _pack_slave_address(self, address: int) -> None:
        ...
        self.slave_address = (self.slave_address & ~((1 << self.additional_address_bits) - 1) - 1) | (additional_address << 1)

    def _poll_ready_status(self, timeout: float = 0.1) -> bool:
        pass
    ...
    def read(self, address: int, length: int) -> bytes:
        ...
        if self.memory_type == MemoryType.EEPROM:
            if not self._poll_ready_status(timeout=0.1):
                raise TimeoutError(f"Timeout while waiting for EEPROM ready status before reading from address {address}")
    ...
    def _flush_section(self, section_start: int, section_end: int) -> None:
            ...
            if self.memory_type == MemoryType.EEPROM:
                if not self._poll_ready_status(timeout=0.1):
                    raise TimeoutError(f"Timeout while waiting for EEPROM ready status before writing to address {section_start}")
            ...
            for i in range(request_count):
                ...
                request = I2cMasterRequest(
                    slave_addr=self.slave_address,
                    write_data=address_bytes + data,
                    read_size=0
                )
                rid = self.interface.send_request(request=request)
                response = self.interface.wait_for_response(request_id=rid, timeout=0.1)
                if response.status_code != I2cStatusCode.SUCCESS:
                    raise ValueError(f"Failed to flush memory at address {section_start}: {response.status_code}")
                
                if self.memory_type == MemoryType.EEPROM:
                    if not self._poll_ready_status(timeout=0.1):
                        raise TimeoutError(f"Timeout while waiting for EEPROM ready status after writing to address {current_address}")

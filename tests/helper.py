...
def generate_slave_config_requests(
    min_addr: int, max_addr: int, min_size: int, max_size: int, count: int, with_read=True
) -> list[I2cSlaveRequest]:
    ...
    for _ in range(count):
        ...
        if with_read:
            read_request = I2cSlaveRequest(
                write_addr=0, write_data=bytes(), read_addr=mem_addr, read_size=len(data_bytes)
            )
            requests.append(read_request)
    return requests


def generate_master_write_read_requests(
    slave_addr: int, min_addr: int, max_addr: int, min_size: int, max_size: int, count: int
) -> list[I2cMasterRequest]:
    if min_addr < 0 or max_addr < 0:
        raise ValueError("Address must be non-negative!")
    if min_addr >= max_addr:
        raise ValueError("Min address must be less than max address!")
...
def i2c_send_request(i2c_int: I2cInterface, request_queue: list[I2cMasterRequest | I2cSlaveRequest]):
    if len(request_queue) and i2c_int.can_accept_request(request_queue[0]):
        ...
        if isinstance(request, I2cMasterRequest):
            print(
                "Send master({}) reqeust (id: {}, w_addr: '{}', w_data: {} ({}), r_size: {})".format(
                    i2c_int.i2c_id.value,
                    rid,
                    request.write_data[:2].hex(),
            ...
            print(
                "Send slave({}) request (id: {}, w_addr: '{}', w_data: {} ({}), r_addr: '{}', r_size: {})".format(
                    i2c_int.i2c_id.value,
                    rid,
                    request.write_addr,
                    request.write_data,
                    len(request.write_data),
                    request.read_addr,
                    request.read_size,
                )
            )
...
def verify_master_write_read_requests(i2c_int: I2cInterface) -> list[I2cMasterRequest]:
    ...
    for request in complete_requests:
        if request.status_code != I2cStatusCode.SUCCESS:
            print("Master request (id: {}) failed with status code: {}".format(request.request_id, request.status_code))
        ...
        if request.read_size == 0:  # Write request
            ...
            if request.read_data != previous_write_request.write_data[2:]:
                print(
                    "Read data does not match write data for request id: {} \n\tW-data: {} ({})\n\tR-data: {} ({})".format(
                        request.request_id,
                        previous_write_request.write_data[2:],
                        len(previous_write_request.write_data[2:]),
                        request.read_data,
                        len(request.read_data),
                    )
                )
